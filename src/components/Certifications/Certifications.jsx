...
const Certifications = () => {
  ...
  // Normalize legacy shapes to a unified media array:
  // media: [{ type: 'image'|'video', src, poster? }, ...]
  const mediaFor = (item) => {
    if (!item) return [];
    if (Array.isArray(item.media) && item.media.length) return item.media;
    if (Array.isArray(item.images) && item.images.length)
      return item.images.map((s) => ({ type: "image", src: s }));
    if (item.image) {
      if (Array.isArray(item.image)) return item.image.map((s) => ({ type: "image", src: s }));
      return [{ type: "image", src: item.image }];
    }
    return [];
  };

  const imagesFor = (item) => mediaFor(item).filter((m) => m.type === "image").map((m) => m.src);

  const previewFor = (item) => {
    const first = mediaFor(item)[0];
    if (!first) return "";
  ...
  // pan handlers (mouse) - only for images when zoom > 1
  const onMouseDown = (e) => {
    ...
    >
      <div className="text-center mb-16">
        ...
        <p className="text-gray-400 mt-4 text-lg font-semibold">
          A showcase of my certifications and courses.
        </p>
      </div>

      <div className="grid gap-12 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
        {certificationsData.map((cert) => {
          const preview = previewFor(cert) || "";
          return (
            <div
              key={cert.id}
            ...
            >
              ...
              <div className="p-6">
                ...
                <p className="text-gray-500 mb-4 pt-4 line-clamp-3">{cert.description}</p>
                <div className="mb-4">
            ...
            <div className="flex flex-col">
              ...
              >
                ...
                {currentMedia?.type === "image" ? (
                  ...
                  >
                    {Array.isArray(currentMedia.src)
                      ? currentMedia.src.map((s, i) => (
                          <source
                            key={i}
                            src={s}
                            type={s.endsWith(".mp4") ? "video/mp4" : "video/quicktime"}
                          />
                        ))
                      : (
                        <source
